...
#
t <- read.table("multovl.anc.txt",header=TRUE,
...
HTML(tt.html,file="multovl.anc.num.html",append=FALSE)
#
if ( dim(tt)[1] == "1") {
  ts <- tt
} else {
...
#
jpeg("multovl.anc.num.jpeg")
plotit("overlaps")
dev.off()
print("plotted multovl.anc.num.jpeg")
pdf("multovl.anc.num.pdf")
plotit("overlaps")
dev.off()
print("plotted multovl.anc.num.pdf")

#
cs <- colSums(tt)/100
tp <- t(apply(tt,1,'/',cs))
tp.html <- rbind(tp,apply(tp,2,sum))
HTML(tp.html,file="multovl.anc.perc.html",append=FALSE)
if (dim(tt)[1] == "1") {
  ts <- tp
} else {
...
#
jpeg("multovl.anc.perc.jpeg")
plotit("percentages")
dev.off()
print("plotted multovl.anc.perc.jpeg")
pdf("multovl.anc.perc.pdf")
plotit("percentages")
dev.off()
print("plotted multovl.anc.perc.pdf")
