...
#
t <- read.table("multovl.strictAgg.anc.txt",header=TRUE,
...
HTML(tt.html,file="multovl.strictAgg.anc.num.html",append=FALSE)
#
if ( dim(tt)[1] == "1") {
  ts <- tt
} else {
...
#
jpeg("multovl.strictAgg.anc.num.jpeg")
plotit("overlaps")
dev.off()
print("plotted multovl.strictAgg.anc.num.jpeg")
pdf("multovl.strictAgg.anc.num.pdf")
plotit("overlaps")
dev.off()
print("plotted multovl.strictAgg.anc.num.pdf")

#
cs <- colSums(tt)/100
tp <- t(apply(tt,1,'/',cs))
tp.html <- rbind(tp,apply(tp,2,sum))
HTML(tp.html,file="multovl.strictAgg.anc.perc.html",append=FALSE)
if (dim(tt)[1] == "1") {
  ts <- tp
} else {
...
#
jpeg("multovl.strictAgg.anc.perc.jpeg")
plotit("percentages")
dev.off()
print("plotted multovl.strictAgg.anc.perc.jpeg")
pdf("multovl.strictAgg.anc.perc.pdf")
plotit("percentages")
dev.off()
print("plotted multovl.strictAgg.anc.perc.pdf")
