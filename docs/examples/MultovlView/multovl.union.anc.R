...
#
t <- read.table("multovl.union.anc.txt",header=TRUE,
...
HTML(tt.html,file="multovl.union.anc.num.html",append=FALSE)
#
if ( dim(tt)[1] == "1") {
  ts <- tt
} else {
...
#
jpeg("multovl.union.anc.num.jpeg")
plotit("overlaps")
dev.off()
print("plotted multovl.union.anc.num.jpeg")
pdf("multovl.union.anc.num.pdf")
plotit("overlaps")
dev.off()
print("plotted multovl.union.anc.num.pdf")

#
cs <- colSums(tt)/100
tp <- t(apply(tt,1,'/',cs))
tp.html <- rbind(tp,apply(tp,2,sum))
HTML(tp.html,file="multovl.union.anc.perc.html",append=FALSE)
if (dim(tt)[1] == "1") {
  ts <- tp
} else {
...
#
jpeg("multovl.union.anc.perc.jpeg")
plotit("percentages")
dev.off()
print("plotted multovl.union.anc.perc.jpeg")
pdf("multovl.union.anc.perc.pdf")
plotit("percentages")
dev.off()
print("plotted multovl.union.anc.perc.pdf")
